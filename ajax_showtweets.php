...
//    return tweets to tweets display area
        ...
        foreach ($arrTweetList as $key => $value) {
            if ($key == "Time") {
                foreach ($value as $v) {
                    array_push($time_tags, $v);
                    $sql = "SELECT `id`,`retweet_count` FROM `TaipeiMayor_main` WHERE `created_at` LIKE '" . $v . "%'";
                    $stmt = $dbh->prepare($sql);
                    $stmt->execute();
                    while ($arrQue = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($key == "Keywords") {
                foreach ($value as $v) {
                    array_push($keywords_tags, $v);
                    $sql = "SELECT `id`,`retweet_count` FROM `TaipeiMayor_main` WHERE `text` LIKE '%" . $v . "%'";
                    $stmt = $dbh->prepare($sql);
                    if (!$stmt->execute()) {
                        throw new Exception("nothing");
                    }
            ...
            if ($key == "Users") {
                foreach ($value as $v) {
                    array_push($users_tags, $v);
                    $sql = "SELECT `id`,`retweet_count` FROM `TaipeiMayor_main` WHERE `from_user_name` LIKE '" . $v . "'";
                    $stmt = $dbh->prepare($sql);
                    $stmt->execute();
                    while ($arrQue = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        if (array_key_exists($arrQue['id'], $tweetRtcnt)) {
...
//    return tweets to tweets display area
        ...
        foreach ($arrTweetList as $key => $value) {
            if ($key == "Time") {
                foreach ($value as $v) {
                    array_push($time_tags, $v);
                    $sql = "SELECT `id`, DATE_FORMAT(`created_at`,'%Y-%m-%d %H:%i') created_at FROM `TaipeiMayor_main` WHERE `created_at` LIKE '" . $v . "%'";
                    $stmt = $dbh->prepare($sql);
                    $stmt->execute();
                    while ($arrQue = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            if ($key == "Keywords") {
                foreach ($value as $v) {
                    array_push($keywords_tags, $v);
                    $sql = "SELECT `id`,DATE_FORMAT(`created_at`,'%Y-%m-%d %H:%i') created_at FROM `TaipeiMayor_main` WHERE `text` LIKE '%" . $v . "%'";
                    $stmt = $dbh->prepare($sql);
                    if (!$stmt->execute()) {
                        throw new Exception("nothing");
                    }
            ...
            if ($key == "Users") {
                foreach ($value as $v) {
                    array_push($users_tags, $v);
                    $sql = "SELECT `id`,DATE_FORMAT(`created_at`,'%Y-%m-%d %H:%i') created_at FROM `TaipeiMayor_main` WHERE `from_user_name` LIKE '" . $v . "'";
                    $stmt = $dbh->prepare($sql);
                    $stmt->execute();
                    while ($arrQue = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        if (array_key_exists($arrQue['id'], $tweetTime)) {
